...
const RestaurantsOrder = () => {
  ...
  const decodedQueryString = decodeURIComponent(location.search);
  const searchParams = new URLSearchParams(decodedQueryString);
  const filterValue = searchParams.get("order_status"); // CORRECTION: Match backend parameter name
  ...
  const handleFilter = (e, value) => {
    ...
    if (value === "all") {
      ...
      searchParams.set("order_status", e.target.value); // CORRECTION: Match backend parameter name
    }
